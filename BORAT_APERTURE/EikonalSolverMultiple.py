...
def BoundaryEventES(t, y):

    point = pv.PointSet(np.array(y[0:3], dtype=float))
    dist = point.compute_implicit_distance(GlobalData.BoundaryES)
    flag = dist['implicit_distance'][0]

    return flag


BoundaryEventES.direction = 1
BoundaryEventES.terminal = True


###################################################################

def BoundaryEventPEC(t, y):
...
BoundaryEventPEC.direction = 1
BoundaryEventPEC.terminal = True
...
def IntegrateEikonal(y0):
    ...
    if _Reflections and _CutoffFlag:
        eventsIntegration = (BoundaryEventES,BoundaryEventPEC, ReflectionEvent, CutoffEvent)

    elif not _Reflections and _CutoffFlag:

        eventsIntegration = (BoundaryEventES,BoundaryEventPEC, CutoffEvent)

    elif _Reflections and not _CutoffFlag:

        eventsIntegration = (BoundaryEventES,BoundaryEventPEC, ReflectionEvent)

    else:
        eventsIntegration = (BoundaryEventES,BoundaryEventPEC)
...
def RefractiveIndexInterpolation(xyz):
    ...
    if GlobalData.Solver.Ionized_solution:
        ...
        if flag < 0:
            ...
            _, index = GlobalData.kdtree.query(xyz, k=32)
...
def RefractiveIndexInterpolation2(xyz):
