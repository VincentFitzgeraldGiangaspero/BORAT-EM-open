...
def FinalRay(RaySolutions, Ray0, rho):
    ...
    pointsRay = np.column_stack((RaySolutions[0, -1, :], RaySolutions[1, -1, :], RaySolutions[2, -1, :]))
    ...
    RayPolyData["K"] = np.column_stack([RayPolyData["kx"], RayPolyData["ky"], RayPolyData["kz"]])

    RayPolyData["Edir"] = np.column_stack([RayPolyData["ex"], RayPolyData["ey"], RayPolyData["ez"]])

    RayPolyData["DF"] = rho / (rho + RayPolyData["ds"])

    RayPolyData["Efield"] = np.column_stack(
        [
            Ray0["E0"][:, 0] * RayPolyData["DF"],
...
def CompleteRay(RaySolutions, Ray0, rho):
    ...
    for iRay in range(totalRays):
        ...
        pointsRay = np.column_stack((RaySolutions[0, :, iRay], RaySolutions[1, :, iRay], RaySolutions[2, :, iRay]))

        lines = np.column_stack((np.full(nSteps - 1, 2), np.arange(nSteps - 1), np.arange(nSteps - 1) + 1))
...
def FinalTubes(RaySolutions, RayTube0, rho):
    ...
    for nTube in range(totalTubes):
        ...
        TubesFinalN[nTube] = (RaySolutions[11, -1, ray1] + RaySolutions[11, -1, ray2] + RaySolutions[11, -1, ray3]) / 3

        TubesFinalPhase[nTube] = (RaySolutions[9, -1, ray1] + RaySolutions[9, -1, ray2] + RaySolutions[9, -1, ray3]) / 3

        TubesFinalds[nTube] = (RaySolutions[10, -1, ray1] + RaySolutions[10, -1, ray2] + RaySolutions[10, -1, ray3]) / 3

        TubesFinalEvector[0, nTube] = (
            RaySolutions[6, -1, ray1] + RaySolutions[6, -1, ray2] + RaySolutions[6, -1, ray3]
        ) / 3

        TubesFinalEvector[1, nTube] = (
            RaySolutions[7, -1, ray1] + RaySolutions[7, -1, ray2] + RaySolutions[7, -1, ray3]
        ) / 3

        TubesFinalEvector[2, nTube] = (
            RaySolutions[8, -1, ray1] + RaySolutions[8, -1, ray2] + RaySolutions[8, -1, ray3]
        ) / 3

        TubesFinalKvector[0, nTube] = (
            RaySolutions[3, -1, ray1] + RaySolutions[3, -1, ray2] + RaySolutions[3, -1, ray3]
        ) / (3 * TubesFinalN[nTube])

        TubesFinalKvector[1, nTube] = (
            RaySolutions[4, -1, ray1] + RaySolutions[4, -1, ray2] + RaySolutions[4, -1, ray3]
        ) / (3 * TubesFinalN[nTube])

        TubesFinalKvector[2, nTube] = (
            RaySolutions[5, -1, ray1] + RaySolutions[5, -1, ray2] + RaySolutions[5, -1, ray3]
        ) / (3 * TubesFinalN[nTube])

        # this is important, check it all the time otherwise the triangle is not the right one

        triangle = pv.Triangle([RaySolutions[:3, -1, ray1], RaySolutions[:3, -1, ray2], RaySolutions[:3, -1, ray3]])
        ...
        if TubesFinalArea[nTube] == 0:
            ...
            TubesDF[nTube] = np.sqrt(TubesInitialArea[nTube] / TubesFinalArea[nTube])
    ...
    FinalRayTube = pv.PolyData(np.transpose(RaySolutions[0:3, -1, :]), RayTube0.faces)
    ...
    FinalRayTube.cell_data["K"] = np.column_stack([FinalRayTube["kx"], FinalRayTube["ky"], FinalRayTube["kz"]])

    FinalRayTube.cell_data["Edir"] = np.column_stack([FinalRayTube["ex"], FinalRayTube["ey"], FinalRayTube["ez"]])

    FinalRayTube["DF3"] = np.sqrt(TubesInitialArea[:] / FinalRayTube["Area"])
    ...
    FinalRayTube.cell_data["|E|"] = FinalRayTube["DF"] * RayTube0["|E0|"] * np.sqrt(FinalRayTube.cell_data["n"])

    FinalRayTube.cell_data["Efield"] = np.column_stack(
        [
            FinalRayTube["|E|"] * FinalRayTube["ex"],
            FinalRayTube["|E|"] * FinalRayTube["ey"],
    ...
    for iTube in range(totalTubes):
        ...
        _, theta, phi = Functions.Cart2sphere(xiyizi.points[iTube, 0], xiyizi.points[iTube, 1], xiyizi.points[iTube, 2])
        ...
        SphericalE[0, iTube] = np.dot(FinalRayTube.cell_data["Efield"][iTube], r_versor)
        SphericalE[1, iTube] = np.dot(FinalRayTube.cell_data["Efield"][iTube], theta_versor)
        SphericalE[2, iTube] = np.dot(FinalRayTube.cell_data["Efield"][iTube], phi_versor)
