...
if __name__ == "__main__":
    ...
    kdtree = KDTree(np.c_[MeshPoints[:, 0], MeshPoints[:, 1], MeshPoints[:, 2]])
    ...
    if Solver.Parallel:
        ...
        if Solver.Parallel_Method == "joblib":
            ...
            EikonalSolutions = Parallel(n_jobs=Solver.nProcess)(delayed(EikonalParallel)(k) for k in range(0, totalRay))
        ...
        for y0 in tqdm(InitialConditions):
            ...
            resultIntegration = EikonalSolver.Solve(Solver, y0)
    ...
    print("Saving  Ray Tracing solution in PLK file: ", outputFolder + "/RayTracing.plk", " \n")
    ...
    FinalRayTube.rotate_vector([0, 1, 0], 57, inplace=True, transform_all_input_vectors=True)

    FinalRayTube.save(outputFolder + "/" + "RayTubeSolution_rotated" + CaseName + ".vtk")
    ...
    print("Saving  SBR solution in PLK file: ", outputFolder + "/SBR.plk", " \n")
