...
class SzpltData(dict):
    ...
    def _tecZoneFaceNbrGetConnections(self, zone_n):
        ...
        if are64Bit:
            faceConnections = self._return_n_array(
                self.dll.tecZoneFaceNbrGetConnections64, ctypes.c_long, numFaceValues, zone_n
            )
        else:
            faceConnections = self._return_n_array(
                self.dll.tecZoneFaceNbrGetConnections, ctypes.c_int, numFaceValues, zone_n
            )
    ...
    def GenerateDataFromOtherFormat(self, dataset):
        ...
        loc_pass = [
            self.judge_valuelocation_passive(zone, vname, name0) for zone in self.keys() for vname in self[zone].keys()
        ]
            ...
            if info["numFaceConnections"] > 0:
                ...
                if faceConnections.itemsize == 8:
                    self._write_data_all_type(
                        self.dll.tecZoneFaceNbrWriteConnections64, faceConnections.ctypes, outputZone
                    )
                else:
                    self._write_data_all_type(
                        self.dll.tecZoneFaceNbrWriteConnections32, faceConnections.ctypes, outputZone
                    )
...
def zone_write_double_values(file_handle, zone, var, values):
    ...
    ret = tecio.tecZoneVarWriteDoubleValues(file_handle, zone, var, 0, len(values), values_ptr)
    if ret != 0:
